...
@Component
public class FecEventCctvScheduler8 {
	...
	public void Execute() {
		...
		System.out.println (" 부산경남본부 CCTV 이벤트 가져오기 시작");	
		...
		try { 
		    ...
		    try {
		    	list = fecTotlInsrTot.checkCCTV("N01801");
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
				System.out.println ("부산경남본부 CCTV 테이블 조회 실패");
			}
...
//					System.out.println( "CCTV Count  :  " + cctvUrlChk.getServiceUrl() );
			...
			// CCTV 이벤트 데이터 삭제(관제 DB)
	 	    try { 
	 	    	fecTotlInsr.fecDeleteCctv("N01801");
	 		} catch (Exception e) {
	 			// TODO Auto-generated catch block
	 			e.printStackTrace();
	 			System.out.println ("부산경남본부 CCTV 이벤트 데이터 삭제 실패");
	 		}
		    ...
		    // CCTV 이벤트 데이터 추가(관제 DB)
	 	    try { 
	 			...
	 			// TODO Auto-generated catch block
	 			e.printStackTrace();
	 			System.out.println ("부산경남본부 CCTV 이벤트 데이터 추가 실패");
	 		} 	
...
//		if (bEvent){
//			System.out.println ("부산경남본부 CCTV 이벤트 발생");
