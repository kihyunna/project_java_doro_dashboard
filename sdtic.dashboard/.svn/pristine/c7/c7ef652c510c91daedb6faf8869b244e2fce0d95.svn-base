<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ taglib prefix="c"       uri="http://java.sun.com/jsp/jstl/core"                     %>

<!DOCTYPE html>
<html lang="ko">
<head>
<title></title>
<meta charset="utf-8">
<title>점검계획대비 점검율</title>
<!-- including ECharts file -->
<c:set var="contextPath" value="${pageContext.request.contextPath}" />

<script type="text/javascript" src="${contextPath}/js/echarts/dist/echarts.min.js"></script>
<script type="text/javascript" src="${contextPath}/js/echarts/etc/echarts-gl.min.js"></script>

<script type="text/javascript" src="${contextPath}/js/common/jquery-2.1.4.min.js"></script>
<script type="text/javascript" src="${contextPath}/js/echarts/dist/world.js"></script>
<script type="text/javascript" src="${contextPath}/js/echarts/theme/dark.js"></script>
</head>
<body>
    <!-- prepare a DOM container with width and height -->
    <div id="main" style="width:100%; height:500px"></div>
	<div id = "idDiv" style="top:30px; width:100%;height: 30px;z-index:102;position: absolute;">
	    <div id = "select1lv" style="float:right;"></div>
	</div>
	<script type="text/javascript">

	var option = {
		    baseOption: {
		        tooltip: {
		        },
		        timeline: {
		            show: false,
		            left: 20,
		            width: 800,
		            axisType: 'category',
		            autoPlay: true,
		            currentIndex: 0,
		            playInterval: 2000,
		            currentIndex:0,
		            label: {
		                normal: {
		                    show: true,
		                    interval: 'auto',
		                    formatter: '{value}',
		                },
		            },
		            data: [],
		        },
		        xAxis: {
		            max: maxData,
		            splitLine: {show: false},
		            offset: 10,
		            axisLine: {
		                lineStyle: {
		                    color: '#999'
		                }
		            },
		            axisLabel: {
		                margin: 10
		            }
		        },
		        yAxis: {
		            data: [],
		            inverse: false,
		            axisTick: {show: false},
		            axisLine: {show: false},
		            axisLabel: {
		                margin: 10,
		                textStyle: {
		                    color: '#999',
		                    fontSize: 16
		                }
		            }
		        },
		        grid: {
		            top: 'center',
		            height: 300,
		            left: 80,
		            right: 100
		        }
		    },
		    series: [],
		    options : []
		};
	
	var myChart = echarts.init(document.getElementById('main'), 'dark');
	myChart.setOption(option);
	
	var spirit = 'image://data:image/png;base64,';
	var spanner = 'image://data:image/png;base64,';
	var maxData = 50;
	var maxCount = '';

	/*
	var xData = {
			'전체': ['부산경남','대구경북','광주전남','전북', '대전충남', '충북', '강원','수도권'],
			'901':['시흥','화성','동서울','경기광주','수원','군포 ','인천'],
		    '902':['양양','춘천','홍천','강릉','대관령','원주','이천'],
		    '958':['상주','보은','진천','엄정','충주','제천'],
		    '903':['부여','공주','당진','영동','대전','천안'],
		    '927':['진안','보령','논산','무주','부안','전주'],
		    '904':['보성','구례','담양','함평','남원','순천','광주'],
		    '905':['청송','성주','영천','영주','군위','고령','대구','구미'],
		    '906':['경주','고성','산청','양산','진주','창원','창녕','울산']
	};
	   
	   
	   
	   var databeast = {
	         '교량': [5, 12, 10, 6, 8, 14, 7, 9 ],
	         '터널': [12, 9, 15, 3, 12, 8, 17, 8],
	         '암거': [5, 13, 13, 7, 4, 6, 13, 16]

	      };
	   var databeauty = {
	         '교량': [25, 15, 13, 16, 18, 14, 13, 10],
	         '터널': [18, 29, 20, 13, 22, 18, 27, 18],
	         '암거': [12, 20, 18, 25, 14, 10, 23, 20]
	         
	   };
	   
	   */
	   var xData = {};
	   var databeast = {};
	   var databeauty = {};
	   
	    var timeLineData = ['교량', '터널', '암거'];
	   
	   var temp0 = [];
	   var temp1 = [];
	   var temp2 = [];

	   
	   function getBucMntmngProfitToPlanViewAjax(){
		   
		   var tmp =  $("#upperhdqrtrs").val();
		   
		   var seriesData='';
		   myChart.clear();
		   option = {
		            baseOption: {
		                tooltip: {
		                },
		                timeline: {
		                    show: false,
		                    left: 20,
		                    width: 800,
		                    axisType: 'category',
		                    autoPlay: true,
		                    currentIndex: 0,
		                    playInterval: 2000,
		                    currentIndex:0,
		                    label: {
		                        normal: {
		                            show: true,
		                            interval: 'auto',
		                            formatter: '{value}',
		                        },
		                    },
		                    data: [],
		                },
		                xAxis: {
		                    //max: maxData,
		                    splitLine: {show: false},
		                    offset: 10,
		                    axisLine: {
		                        lineStyle: {
		                            color: '#999'
		                        }
		                    },
		                    axisLabel: {
		                        margin: 10
		                    }
		                },
		                yAxis: {
		                    data: [],
		                    inverse: false,
		                    axisTick: {show: false},
		                    axisLine: {show: false},
		                    axisLabel: {
		                        margin: 10,
		                        textStyle: {
		                            color: '#999',
		                            fontSize: 16
		                        }
		                    }
		                },
		                grid: {
		                    top: 'center',
		                    height: 300,
		                    left: 80,
		                    right: 100
		                }
		            },
		            series: [],
		            options : []
		        };
		   
		   
		   $.ajax({
	            url : "${pageContext.request.contextPath}/buc/bucMntmngProfitToPlanAjax.do",
	            async :  true,
	            dataType : 'json',
	            data: {
	            	'bonbu_gubun' : tmp
	            	
	            },

	            success :function(json) {
	            	//Xdata
		       		 
	            	xData[tmp] = new Array();	
	            	
	            	for(var i=0; i<json.data.length; i++)
                    {
	            		xData[tmp][i] = json.data[i].bonbu;
	            		
                    }
	            	
	            
	            	
	            	
	            	for(var i=0; i<timeLineData.length; i++)
                    {
	            		databeast[timeLineData[i]] = new Array();
            			databeauty[timeLineData[i]] = new Array();
            			
	            		for(var j=0; j<maxCount; j++)
	                    {	            			
	            			//교량
	            			if(i == 0){
	            				if(j < json.data.length){
	            					//alert(1);
	            					databeast[timeLineData[i]][j] = json.data[j].gs_data;
		            				databeauty[timeLineData[i]][j] = json.data[j].gp_data;
	            				}else{
	            					databeast[timeLineData[i]][j] = null;
		            				databeauty[timeLineData[i]][j] = null;
	            				}
	            				
	            			}
	            			
	            			//터널
	            			if(i == 1){
	            				if(j < json.data.length){
	            					//alert(2);
	            				databeast[timeLineData[i]][j] = json.data[j].ts_data;
	            				databeauty[timeLineData[i]][j] = json.data[j].tp_data;
	            				}else{
	            					databeast[timeLineData[i]][j] = null;
		            				databeauty[timeLineData[i]][j] = null;
	            				}
	            			}
	            			
	            			//암거
	            			if(i == 2){
	            				if(j < json.data.length){
	            					//alert(3);
	            				databeast[timeLineData[i]][j] = json.data[j].as_data;
	            				databeauty[timeLineData[i]][j] = json.data[j].ap_data;
			                    }else{
		        					databeast[timeLineData[i]][j] = null;
		            				databeauty[timeLineData[i]][j] = null;
		        				}
	                    	}
                    	}
	            		
	            		for(var j = 0 ; j < maxCount; j++){
	                        switch (i) {
	                        case 0:
	                            temp0[j] = ((databeast['교량'][j] / databeauty['교량'][j]) * 100).toFixed(1);
	                            
	                            break;
	                        case 1:
	                            temp1[j] = ((databeast['터널'][j] / databeauty['터널'][j]) * 100).toFixed(1);
	                           
	                            break;
	                            
	                        case 2:
	                            temp2[j] = ((databeast['암거'][j] / databeauty['암거'][j]) * 100).toFixed(1);
	                            
	                            break;
	                        default:
	                            break;
	                        }
	                    } 
	            		
          	
	            		//temp2 = databeauty[timeLineData[i]];
	            		
	            		//alert(xData[tmp]);

	            	        option.baseOption.timeline.data = timeLineData;
	            	        option.baseOption.yAxis.data = xData[tmp];
	            	        option.options.push({
	            	        	title: {
	            					show: true,
	            					text: timeLineData[i]
	            				},
	            	            series: [{
	            	                // current data
	            	                type: 'pictorialBar',
	            	                symbol: spanner,
	            	                symbolRepeat: 'true',
	            	                symbolMargin: '5%',
	            	                symbolClip: false,
	            	                symbolSize: 30,
	            	                //symbolBoundingData: maxData,
	            	                data: databeast[timeLineData[i]],
	            	                z: 10
	            	            },{
	                                // full data
	                                type: 'pictorialBar',
	                                itemStyle: {
	                                    normal: {
	                                        opacity: 0.2
	                                    }
	                                },
	                                label: {
	                                    normal: {
	                                        show: true,
	                                        /* formatter: function(params) {
	                                            return params.value +'%';
	                                        } */
	                                    },
	                                        position: 'right',
	                                        offset: [10, 0],
	                                        textStyle: {
	                                            color: 'white',
	                                            fontSize: 18
	                                        }
	                                    
	                                },
	                                animationDuration: 0,
	                                symbolRepeat: 'true',
	                                symbolMargin: '5%',
	                                symbol: spirit,
	                                symbolSize: 30, 
	                                //symbolBoundingData: maxData,
	                                data: [{
	                                	value: databeauty[timeLineData[i]][0], name:'%',
	                                    label: {
	                                    	   normal: {
	                                    		    formatter:function(v){
	                                    		        var tempdata = 0;
	                                    		        
                                                        tempdata = temp0[0] + '%';

                                                    
                                                        return tempdata;
                                                },
                                                textStyle: {
                                                    color: '#fff',
                                                    fontSize: 18
                                               },
                                                position: 'right'
                                           }
                                        }
	                                },
	                                {
                                        value: databeauty[timeLineData[i]][1], name:'%',
                                        label: {
                                               normal: {
                                                    formatter:function(v){
                                                        var tempdata = 0;

                                                        tempdata = temp0[1] + '%';

                                                    
                                                        return tempdata;
                                                },
                                                textStyle: {
                                                    color: '#fff',
                                                    fontSize: 18
                                               },
                                                position: 'right'
                                           }
                                        }
                                    },
                                    {
                                        value: databeauty[timeLineData[i]][2], name:'%',
                                        label: {
                                               normal: {
                                                    formatter:function(v){
                                                        var tempdata = 0;

                                                        tempdata = temp0[2] + '%';

                                                    
                                                        return tempdata;
                                                },
                                                textStyle: {
                                                    color: '#fff',
                                                    fontSize: 18
                                               },
                                                position: 'right'
                                           }
                                        }
                                    },
                                    {
                                        value: databeauty[timeLineData[i]][3], name:'%',
                                        label: {
                                               normal: {
                                                    formatter:function(v){
                                                        var tempdata = 0;

                                                        tempdata = temp0[3] + '%';

                                                    
                                                        return tempdata;
                                                },
                                                textStyle: {
                                                    color: '#fff',
                                                    fontSize: 18
                                               },
                                                position: 'right'
                                           }
                                        }
                                    },
                                    {
                                        value: databeauty[timeLineData[i]][4], name:'%',
                                        label: {
                                               normal: {
                                                    formatter:function(v){
                                                        var tempdata = 0;

                                                        tempdata = temp0[4] + '%';

                                                    
                                                        return tempdata;
                                                },
                                                textStyle: {
                                                    color: '#fff',
                                                    fontSize: 18
                                               },
                                                position: 'right'
                                           }
                                        }
                                    },
                                    {
                                        value: databeauty[timeLineData[i]][5], name:'%',
                                        label: {
                                               normal: {
                                                    formatter:function(v){
                                                        var tempdata = 0;

                                                        tempdata = temp0[5] + '%';

                                                    
                                                        return tempdata;
                                                },
                                                textStyle: {
                                                    color: '#fff',
                                                    fontSize: 18
                                               },
                                                position: 'right'
                                           }
                                        }
                                    },
                                    {
                                        value: databeauty[timeLineData[i]][6], name:'%',
                                        label: {
                                               normal: {
                                                    formatter:function(v){
                                                        var tempdata = 0;

                                                        tempdata = temp0[6] + '%';

                                                    
                                                        return tempdata;
                                                },
                                                textStyle: {
                                                    color: '#fff',
                                                    fontSize: 18
                                               },
                                                position: 'right'
                                           }
                                        }
                                    },
                                    {
                                        value: databeauty[timeLineData[i]][7], name:'%',
                                        label: {
                                               normal: {
                                                    formatter:function(v){
                                                        var tempdata = 0;

                                                        tempdata = temp0[7] + '%';

                                                    
                                                        return tempdata;
                                                },
                                                textStyle: {
                                                    color: '#fff',
                                                    fontSize: 18
                                               },
                                                position: 'right'
                                           }
                                        }
                                    }],
	                                z: 5
	            	            }]
	            	        })
	            	       
	            	
	            }
	            	myChart.setOption(option);
	            	/*
	                    for(var i=0; i<json.data.length; i++)
	                    {
	                        data1[i] = json.data[i].bonbu;
	                        //data2[i] = json.data[i].bonbucode;
	                        data3[i] = json.data[i].ts_data;
	                        data4[i] = json.data[i].tp_data;
	                        //data5[i] = json.data[i].gs_data;
	                        //data6[i] = json.data[i].gp_data;
	                        //data7[i] = json.data[i].as_data;
	                        //data8[i] = json.data[i].ap_data;
	                        
	                    }
	                    myChart.setOption(option);
	            	 */

	            },error : function(){
	                console.log(arguments);
	            }
	        });
	      

	    }
	  
	

	
	
	 myChart.on('timelinechanged', function (params) {
		 
		 var currentIndex = params.currentIndex;
		 
          for(var j = 0 ; j < databeast['교량'].length; j++){
             switch (currentIndex) {
             case 0:
                 temp0[j] = ((databeast['교량'][j] / databeauty['교량'][j]) * 100).toFixed(1);
                 
                 break;
             case 1:
            	 temp0[j] = ((databeast['터널'][j] / databeauty['터널'][j]) * 100).toFixed(1);
            	
                 break;
                 
             case 2:
            	 temp0[j] = ((databeast['암거'][j] / databeauty['암거'][j]) * 100).toFixed(1);
            	 
                 break;
             default:
                 break;
             }
         } 
         option.baseOption.timeline.currentIndex = currentIndex;
         myChart.setOption(option);
     });
	
	function test() {
		
		
	}
	/*var maxData = 10;
	
	
	var xData = ['부산경남','대구경북','광주전남','전북', '대전충남', '충북', '강원','수도권'];
	var xData2 =['시흥','화성','동서울','경기광주','수원','군포','인천'];
	var xData3 =['양양','춘천','홍천','강릉','대관령','원주','이천'];
	var xData4 =['상주','보은','진천','엄정','충주','제천'];
	var xData5 =['부여','공주','당진','영동','대전','천안'];
	var xData6 =['진안','보령','논산','무주','부안','전주'];
	var xData7 =['보성','구례','담양','함평','남원','순천','광주'];
	var xData8 =['청송','성주','영천','영주','군위','고령','대구','구미'];
	var xData9 =['경주','고성','산청','양산','진주','창원','창녕','울산'];
	
	
	
	var databeast = [5, 5, 3, 6, 8, 4, 5, 9 ];
			//'터널': [2, 9, 5, 3, 2, 8, 7, 8],
			//  '암거': [5, 3, 3, 7, 4, 6, 3, 6]

		
	var databeauty = [15, 12, 10, 16, 18, 14, 13, 10 ];
			//'터널': [11, 19, 10, 13, 12, 18, 17, 18],
			//'암거': [12, 13, 11, 15, 14, 10, 13, 10]
			//'교량': [5, 5, 3, 6, 8, 4, 5, 9 ],
			//'터널': [2, 9, 5, 3, 2, 8, 7, 8],
			//'암거': [5, 3, 3, 7, 4, 6, 3, 6]

	//var timeLineData = ['교량', '터널', '암거'];
	option = {

    xAxis: {
        max: maxData,
        splitLine: {show: false},
        offset: 10,
        axisLine: {
            lineStyle: {
                color: '#999'
            }
        },
        axisLabel: {
            margin: 10
        }
    },
    yAxis: {
        data:['부산경남','대구경북','광주전남','전북', '대전충남', '충북', '강원','수도권'],
        //inverse: true,
        axisTick: {show: false},
        axisLine: {show: false},
        axisLabel: {
            margin: 10,
            textStyle: {
                color: '#999',
                fontSize: 16
            }
        }
    },
    grid: {
        top: 'center',
        height: 200,
        left: 70,
        right: 100
    },
    series: [{
        // current data
        type: 'pictorialBar',
        symbol: spanner,
        symbolRepeat: 'fixed',
        symbolMargin: '5%',
        symbolClip: true,
        symbolSize: 30,
        symbolBoundingData: maxData,
        data: databeast,
        markLine: {
            symbol: 'none',
            label: {
                normal: {
                    formatter: 'max: {c}',
                    position: 'start'
                }
            },
            lineStyle: {
                normal: {
                    color: 'green',
                    type: 'dotted',
                    opacity: 0.2,
                    width: 2
                }
            },
            data: [{
                type: 'max'
            }]
        },
        z: 10
    }, {
        // full data
        type: 'pictorialBar',
        itemStyle: {
            normal: {
                opacity: 0.2
            }
        },
        label: {
            normal: {
                show: true,
                formatter: function (params) {
                    return (params.value / maxData * 100).toFixed(0) + ' %';
                },
                position: 'right',
                offset: [10, 0],
                textStyle: {
                    color: 'green',
                    fontSize: 18
                }
            }
        },
        animationDuration: 0,
        symbolRepeat: 'true',
        symbolMargin: '5%',
        symbol: spirit,
        symbolSize: 30,
        symbolBoundingData: maxData,
        data: databeast,
        z: 5
    }]
};*/
	myChart.setOption(option);
	
	$(window).resize(function(){
	
	    setDivHeight('main');
	
	});
	
	function setDivHeight(objSet)
	{
	
	    var objSet   = document.getElementById(objSet);
	    objSet.style.width  = ($(window).width()) + "px";
	    objSet.style.height  = ($(window).height()) + "px";
	    option.baseOption.timeline.width = ($(window).width()) + "px";
	
	
	    $('#main').width(($(window).width()) + "px");
	    $('#main').height(($(window).height()) + "px");
	
	
	    myChart.resize({
	        width: $(window).width(),
	        height : $(window).height()
	        
	    });
	    myChart.setOption(option);
		
	}
	
	jQuery(document).ready(function($){
	    searchUpperhdqrtrs();
	    
	    $.ajax({
            url : "${pageContext.request.contextPath}/buc/bucMntmngProfitToPlanMaxAjax.do",
            async : false,
            dataType : 'json',

            success :function(json) {
            	for(var i = 0; i<json.data.length; i++){
            	maxCount = json.data[0].bonbuMax;
            	}
            	
            }
	   });
	    
	    function searchUpperhdqrtrs(){
	        $.ajax({
	            type: 'POST',
	            url: "/buc/selectMainInstt1lv.do",
	            dataType: "json",
	            contentType : "application/x-www-form-urlencoded; charset=UTF-8",
	            success: function(data, responseText, statusText, textStatus){
	                $("#select1lv").html("");
	                var contentStr  = "";
	                contentStr += "<select id='upperhdqrtrs' onchange ='getBucMntmngProfitToPlanViewAjax()'style='width:150px;height: 30px;z-index:102;background-color:rgba(50,50,50,0.7);color:#eee;font-size:20px;'>";
	                contentStr += "<option value=''> == 전체 ==</option>";
	                for(var i=0; i<data.data.length; i++)
	                {
	                    contentStr += "    <option value=\'"+data.data[i].intg_dptcd+"\'>"+data.data[i].kor_dptnm+"\</option>";
	                }
	                contentStr += "</select>";
	                $("#select1lv").html(contentStr);

	                getBucMntmngProfitToPlanViewAjax();
	            }
	            ,error : function(request, status, error){  alert("code : " +request.status + "\r\nmessage : " + request.reponseText); }
	            ,beforeSend:function(){}
	            ,complete :function(){}
	        });
	    }
	
	});
	
	
	 function refreshChart(){
		 var tmp =  $("#upperhdqrtrs").val();
		 var seriesData='';
		 if(tmp == ''){
			 tmp = '전체';
		 }
     	
			 for(var j = 0; j < xData[tmp].length; j++){
				 
				seriesData +=
				 "{value:"+ databeauty['교량'][j]+", name:'%',"+
                      "label: {"+
                         "normal: {"+
                             "formatter:function(v){"+
                                 "var tempdata = 0;"+

                                     "tempdata = "+temp0[j] + "'%';"+

                                 
                                 "return tempdata;"+
                                 
                             "},"+
                             "textStyle: {"+
                                " color: '#FFF',"+
                                 "fontSize: 18"+
                            "},"+
                             "position: 'right'"+
                         "}"+
                     "}";
                 
				 if(j < xData[tmp].length -1){
					 seriesData += "},";
				 }
					else{
						 seriesData += "}";
					}
			 }
			
	     	
		 option.baseOption.yAxis.data = xData[tmp];

		// option.options[0].series[1].data[0] = seriesData;
		 myChart.setOption(option);
		 
		 

     }
</script>
	</script>
</body>
</html>
